...
public class PaginaController {
    ...
    public String carregarDashboard(@RequestParam(defaultValue = "7") Integer dias, 
            ...
            if (ultimoEstudo.equals(hoje) || ultimoEstudo.equals(ontem)) {
                ...
                for (int i = 1; i < diasEstudados.size(); i++) {
                    if (diasEstudados.get(i).equals(dataCheck.minusDays(1))) {
                        streak++; dataCheck = diasEstudados.get(i);
                    } else { break; }
                }
            }
        }
        ...
        // --- 3. FILTRAGEM PARA GRÁFICOS ---
        // Filtra por matéria se selecionado
        List<Registro> registrosFiltrados = todos.stream()
            .filter(r -> materiaId == null || r.getAssunto().getMateria().getId().equals(materiaId))
            .collect(Collectors.toList());

        List<Registro> soQuestoes = registrosFiltrados.stream()
            .filter(r -> r.getTema() == null || r.getTema().isEmpty()).collect(Collectors.toList());
        List<Registro> soRedacoes = registrosFiltrados.stream()
            .filter(r -> r.getTema() != null && !r.getTema().isEmpty()).collect(Collectors.toList());
        ...
        List<Double> dadosHorasEstudadas = new ArrayList<>(); // Novo: Horas por dia
        ...
        if (isModoAnual) { 
            ...
            dataInicioCalculada = cursor; // Guarda o início para o gráfico de pizza
            ...
            while (!cursor.isAfter(dataFim)) {
                ...
                // Soma de Tempo (Horas)
                double minutosMes = rMes.stream()
                    .filter(r -> r.getTempo() != null)
                    .mapToDouble(r -> r.getTempo().getHour() * 60 + r.getTempo().getMinute())
                    .sum();
                ...
                dadosHorasEstudadas.add(minutosMes / 60.0); // Converte para Horas
            ...
            while (!cursor.isAfter(dataFim)) {
                ...
                dadosTotal.add(t); dadosAcertos.add(a); dadosErros.add(t - a);
                dadosRedacaoNotas.add((int) mediaRedacao);
                dadosHorasEstudadas.add(minutosDia / 60.0); 

                cursor = cursor.plusDays(1);
            }
        }

        // --- 5. GRÁFICO DE PIZZA (Distribuição por Matéria no Período) ---
        // Filtra registros apenas do período selecionado
        LocalDate dataCorte = dataInicioCalculada;
        List<Registro> registrosDoPeriodo = registrosFiltrados.stream()
                .filter(r -> !r.getData().isBefore(dataCorte))
                .filter(r -> r.getTempo() != null) // Só conta quem tem tempo registrado
                .collect(Collectors.toList());

        // Agrupa e soma minutos por matéria
        ...
        for (Registro r : registrosDoPeriodo) {
            ...
            double minutos = r.getTempo().getHour() * 60 + r.getTempo().getMinute();
            mapaMateriaTempo.put(nomeMat, mapaMateriaTempo.getOrDefault(nomeMat, 0.0) + minutos);
        }

        List<String> pizzaLabels = new ArrayList<>();
        List<Double> pizzaDados = new ArrayList<>();
        
        mapaMateriaTempo.forEach((materia, minutos) -> {
            pizzaLabels.add(materia);
            pizzaDados.add(minutos / 60.0); // Converte para Horas
        });

        // --- ENVIANDO TUDO PARA O MODEL ---
        ...
        model.addAttribute("graficoHorasEstudadas", dadosHorasEstudadas); // Novo
        
        model.addAttribute("pizzaLabels", pizzaLabels); // Novo
        model.addAttribute("pizzaDados", pizzaDados);   // Novo
